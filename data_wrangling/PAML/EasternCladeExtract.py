    ...
    for k in XMid.keys():
        ...
        if k in XMloc_pos.keys():
            for species in genome.keys():
                ...
                div = str(len(temp_str) / 3) # Using the cds eliminated the need to add N's
                print(species, k, len(temp_str), div)
                if findall(r"\.0", div):
                    f.write(">"+species+"\n"+temp_str+"\n")
                elif findall(r"\.3+[0-9]", div):
                    f.write(">"+species+"\n"+temp_str+"NN"+"\n")
                elif findall(r"\.6+[0-9]", div):
                    f.write(">"+species+"\n"+temp_str+"N"+"\n")
            ...
            for species in genome.keys():
                ...
                div = str(len(reverse) / 3) # Using the cds eliminated the need to add N's
                print(species, k, len(reverse), div)
                if findall(r"\.0", div):
                    f.write(">"+species+"\n"+reverse+"\n")
                elif findall(r"\.3+[0-9]", div):
                    f.write(">"+species+"\n"+reverse+"NN"+"\n")
